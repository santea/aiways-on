...
WHERE (
    M.LABEL         LIKE '%보안서약%'
    OR M.LABEL_JSON LIKE '%보안서약%'
)
ORDER BY M.MENU_LEVEL, M.MENU_SEQUENCE;

-- 결과:
-- MENU_ID: AZfTGd-KAACm_Dhq
-- LABEL: SECURITY PLEDGE
-- LABEL_JSON: {"ko_KR":"보안서약서","en_US":"SECURITY PLEDGE"}
-- MENU_LEVEL: 3, MENU_SEQUENCE: 7
-- EXTERNAL_URL: /portal/security/SecurityPledgeList.do
...
-- 변수 선언
-- ============================================================
DECLARE @MENU_ID       NVARCHAR(50) = 'AZfTGd-KAACm_Dhq';
DECLARE @PARENT_ID     NVARCHAR(50) = 'AVdQgpNMAAd_i4eq';  -- 런처관리 그룹
DECLARE @NEW_URL       NVARCHAR(200) = '/admin/launcher/security-pledges';
...
BEGIN
    ...
    SET LABEL          = '보안서약서',
        LABEL_JSON     = '{"ko_KR":"보안서약서","en_US":"SECURITY PLEDGE"}',
        MENU_SEQUENCE  = 31,  -- 플랫폼 전환 상태 관리(30) 바로 다음
        MENU_LEVEL     = 3,
        USE_YN         = '1',
        DELETE_YN      = '0',
...
BEGIN
    ...
    VALUES
    (@MENU_ID, '보안서약서',
     '{"ko_KR":"보안서약서","en_US":"SECURITY PLEDGE"}',
     31, 3, '1', '0', 'ATMQ58N6AABlL532', 'A', '0', @NEW_URL);  -- 플랫폼 전환 상태 관리(30) 바로 다음
...
FROM TN_CF_MENU M
JOIN TN_CF_SYS_RESOURCE SR ON M.MENU_ID = SR.SYS_RESOURCE_ID
WHERE M.MENU_ID = 'AZfTGd-KAACm_Dhq';
