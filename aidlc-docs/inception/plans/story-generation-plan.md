...
## 섹션 4: v2 보강 실행 체크리스트 (2026-09-08, CONSTRUCTION 중 revisit)

> **트리거**: 사용자가 AIways-On 비즈니스 내러티브를 제공하고 "user-story 부분을 보강" 요청.
> **방법**: 내러티브 문단을 `requirements/` 명세와 대조 → 명세에 근거가 있는데 v1이 놓친 것만 스토리화.
> 명세에 근거가 없는 것은 **지어내지 않고 GAP으로 표기**.

### 입력 분석
- [x] 내러티브 7개 절 분해 (배경·액터 A/B·SDLC 예시·MCP·장애대응·지속개선·메신저)
- [x] `requirements/03-state-machine.md` §4.4 자동머지 분기 확인
- [x] `requirements/07-n8n-workflows.md` §2.4~3.14 (요구사항/설계 인터뷰·정의서·설계서·최종보고서) 확인
- [x] `requirements/11-incident-response-agent.md` (MCP 조회·경쟁가설·자동머지 금지) 확인
- [x] `requirements/13-developer-memory-agent.md` §1.2~2.2 (category 5종·토큰 2종·불변 이력) 확인
- [x] `requirements/08-sr-registration-ui.md`, `04-db-schema.md` (`autoPrMerge` 기본값 false) 확인

### 페르소나 보강
- [x] 액터 A ↔ P1 (1:1), B ↔ P2+P3 (1:2 분해) 매핑표 작성
- [x] B 책임의 P2/P3 분해표 — 자동머지 설정 권한이 P3에 있음을 명시
- [x] 비즈니스 기준선 표 (20건/월 · 1일 · 2~3일 · 1주)
- [x] P1 「요구사항 구체화 6축」 / P2 「기술 결정 6항목」 추가
- [x] 새 페르소나를 만들지 않은 근거 기록 (권한 매트릭스 정합 유지)

### 스토리 보강
- [x] U2 +2 — US-U2-09(자동머지 판정·완료요약) · US-U2-10(repo별 실행 정책)
- [x] U3 +7 — US-U3-19(작업공간 컨텍스트) · 20(요구사항 구체화) · 21(정의서) · 22(설계 인터뷰) · 23(설계서) · 24(최종보고서) · 25(메신저 참여)
- [x] U4 +3 — US-U4-08(MCP 원인추적) · 09(리포트 4요소) · 10(장애 지식 축적)
- [x] U5 +2 — US-U5-07(탐색 범위 7종) · 08(규정 승격 admin 승인)
- [x] U6 +3 — US-U6-08(category 5종·불변 이력) · 09(agent 토큰 범위) · 10(실패사례 축적)

### 품질 검증
- [x] 인수 조건 형식 규칙(Q4=C) 준수 — UI는 체크리스트, API·상태머신·시스템 액터는 GWT
- [x] 신규 17건 전부 FR 추적성 명기 (신규 FR 신설 없음 — 기존 FR에 귀속)
- [x] v1 스토리와의 중복 검사 — US-U3-10 vs US-U3-20, US-U6-07 vs US-U6-09 관계 명시
- [x] 내러티브 문단 ↔ 스토리 추적 매트릭스 17행 작성
- [x] GAP-01~07 식별 및 권고 기록
- [x] 유닛 경계 스토리 추가 표기 (US-U4-08·10, US-U5-07·08, US-U6-09)
- [x] U3 집중 심화(25건/34%) 관찰 및 완화안 기록

### 산출물 확정
- [x] `personas.md` v2 섹션 추가 (181 → 255줄)
- [x] `stories.md` v2 섹션 추가 (793 → 1,118줄, 56 → 73 스토리)
- [x] `aidlc-state.md` 갱신 (v2 체크리스트 + GAP 요약표)
- [x] `audit.md` 기록
- [x] ✅ 사용자 승인 완료 (2026-09-08) — GAP-04~07 처리 방향은 별도 결정 대상으로 남김
